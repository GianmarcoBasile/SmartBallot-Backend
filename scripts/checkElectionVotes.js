...
async function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: node checkElectionVotes.js <contractAddress> <electionId>');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('Active:', active, 'HasExpired:', hasExpired, 'EndTime:', new Date(Number(endTime) * 1000).toString());
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      let count = voteCounts && voteCounts[i] !== undefined ? voteCounts[i] : null;
      if (count === null) {
        try {
