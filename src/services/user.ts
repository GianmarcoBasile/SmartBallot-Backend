...
export async function registerUser(
  ...
  const result = await collection.insertOne({
    ...
    last_login: new Date().toLocaleDateString("it-IT"),
  });
  return result;
}
...
export async function findUserByEmail(email: string): Promise<USER | null> {
  const collection: mongoDB.Collection<USER> = await getUsersCollection();
  const userDocument: mongoDB.Document | null = await collection.findOne({
    email,
  });
...
// Search a user by their tax code
export async function findUserByTaxCode(
  tax_code: string,
): Promise<USER | null> {
  const collection: mongoDB.Collection<USER> = await getUsersCollection();
  const userDocument: mongoDB.Document | null = await collection.findOne({
...
// Check if a user is registered by their email or tax code
export async function isUserRegistered(
  ...
  const userByTaxCode = tax_code
    ? await collection.findOne({ tax_code })
    : null;
  return !!(userByEmail || userByTaxCode);
}

// Get all condominiums associated with a user
export async function getUserCondominiums(
  tax_code: string,
): Promise<CONDOMINIUM[]> {
...
// Add a condominium to a user's list of condominiums
export async function addCondominiumToUser(
  tax_code: string,
...
// Get all users associated with a specific condominium
