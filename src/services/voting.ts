...
export async function submitVote(
  ...
  try {
    ...
    // Build proof object as expected by the contract
    ...
    const tx = await condominiumContract.vote(
      electionId,
      optionIndex,
      formattedProof,
    );
    ...
    console.log(
      `Vote submitted for election ${electionId}, option ${optionIndex}, tx: ${tx.hash}`,
    );
    return tx.hash;
  } catch (error) {
...
export async function getElectionDetails(
  ...
  try {
    ...
    const hasExpired =
      details.hasExpired || (details.endTime && details.endTime < currentTime);
...
// Close an election on the blockchain
